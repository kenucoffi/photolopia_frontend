  ...
  return (
    <div className='grid grid-cols-1 ml-18 md:ml-2 h-[500px] border border-blue-300 shadow-lg rounded-xl shadow-blue-200'>
          <div className='flex flex-col'>
            <div className='relative overflow-auto col-span-1 h-[20%]  rounded-xl object-center aspect-square bg-gray-300 hover:bg-gray-400 blur-none '>{big_profile_image?<img src= {url+'/' + big_profile_image} alt='profie' className="absolute object-center w-full h-full"/>:null} <div className='absolute right-4 top-4'><Edit2 onClick={handleBig} size={30} className='text-green-500 cursor-pointer hover:text-blue-500'/></div></div>
            <div className='ml-10 border-4 border-blue-300 p-16 btn btn-circle  mt-[-60px] z-50'><div className="relative overflow-auto  p-15 btn btn-circle">{profile_image?< img src= {url+'/' + profile_image} alt='profie' className="absolute object-center w-full h-full"/>:null}<div className='absolute right-4 top-4'><Edit2 onClick={handleProfile} size={25} className='text-green-500 cursor-pointer hover:text-blue-500'/></div></div></div>
